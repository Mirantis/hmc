...
import (
	...
	"github.com/Mirantis/hmc/internal/util"
...
func getMissingProviders(exposedProviders []string, requiredProviders []string) []string {
	exposedBootstrapProviders := util.SliceToMapKeys[[]string, map[string]struct{}](exposedProviders)
	diff, isSubset := util.DiffSliceSubset[[]string, map[string]struct{}](requiredProviders, exposedBootstrapProviders)
	if !isSubset {
		return diff
	}
	return []string{}
