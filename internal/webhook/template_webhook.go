...
func (v TemplateValidator) isTemplateDeletionAllowed(ctx context.Context, template client.Object) (bool, admission.Warnings, error) {
	...
	if serviceAccountIsEqual(req, v.SystemNamespace, os.Getenv(ServiceAccountEnvName)) {
	...
	if !triggeredByController && templateManagedByHMC(template) {
		return false, admission.Warnings{fmt.Sprintf("The Template is managed by the TemplateManagement and %s", getTemplateChainKind(template))}, nil
	}

	kind := template.GetObjectKind().GroupVersionKind().Kind
	switch kind {
	case v1alpha1.ProviderTemplateKind:
		releaseName := getReleaseForTemplate(template)
		if triggeredByController || releaseName == "" {
			return true, nil, nil
		}
		releaseExists, err := v.releaseExists(ctx, releaseName)
		if err != nil {
			return false, nil, err
		}
		if releaseExists {
			return false, admission.Warnings{fmt.Sprintf("ProviderTemplate object can't be removed since it's part of the %s release", releaseName)}, nil
		}
		return true, nil, nil
	case v1alpha1.ClusterTemplateKind, v1alpha1.ServiceTemplateKind:
		...
		if inUseByChain && !triggeredByController {
	...
	default:
		return false, nil, fmt.Errorf("invalid Template kind. Supported values are: %s, %s and %s", v1alpha1.ProviderTemplateKind, v1alpha1.ClusterTemplateKind, v1alpha1.ServiceTemplateKind)
	}
	return true, nil, nil
}

func getReleaseForTemplate(template client.Object) string {
	return template.GetLabels()[v1alpha1.HMCReleaseLabelKey]
}

func (v TemplateValidator) releaseExists(ctx context.Context, releaseName string) (bool, error) {
	release := &v1alpha1.Release{}
	if err := v.Get(ctx, types.NamespacedName{Name: releaseName}, release); err != nil {
...
func (v TemplateValidator) templateInUseByCluster(ctx context.Context, template client.Object) (bool, error) {
	...
	default:
		return false, fmt.Errorf("invalid Template kind. Supported values are: %s and %s", v1alpha1.ClusterTemplateKind, v1alpha1.ServiceTemplateKind)
	}
