...
func setClusterName(name string) {
	GinkgoT().Setenv(EnvVarManagedClusterName, name)
}
...
func GetUnstructured(templateType templates.Type, conf *config.ClusterTestingConfig) *unstructured.Unstructured {
	GinkgoHelper()

	setClusterName(conf.Name)
	setTemplate(conf.Template)
