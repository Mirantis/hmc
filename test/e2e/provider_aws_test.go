...
var _ = Describe("AWS Templates", Label("provider:cloud", "provider:aws"), Ordered, func() {
	...
	It("should work with an AWS provider", func() {
		for i, providerConfig := range providerConfigs {
			...
			sdName := managedcluster.GenerateClusterName(fmt.Sprintf("aws-%d", i))
			sdTemplate := providerConfig.Standalone.Template
			templateBy(templates.TemplateAWSStandaloneCP, fmt.Sprintf("creating a ManagedCluster %s with template %s", sdName, sdTemplate))

			sd := managedcluster.GetUnstructured(templates.TemplateAWSStandaloneCP, sdName, sdTemplate)
			...
			hdName := managedcluster.GenerateClusterName(fmt.Sprintf("aws-hosted-%d", i))
			hdTemplate := providerConfig.Hosted.Template
			templateBy(templates.TemplateAWSHostedCP, fmt.Sprintf("creating a hosted ManagedCluster %s with template %s", hdName, hdTemplate))
			hd := managedcluster.GetUnstructured(templates.TemplateAWSHostedCP, hdName, hdTemplate)
