...
var _ = Context("Azure Templates", Label("provider:cloud", "provider:azure"), Ordered, func() {
	...
	It("should work with an Azure provider", func() {
		for i, providerConfig := range providerConfigs {
			...
			sdName := managedcluster.GenerateClusterName(fmt.Sprintf("azure-%d", i))
			sdTemplate := providerConfig.Standalone.Template
			templateBy(templates.TemplateAzureStandaloneCP, fmt.Sprintf("creating a ManagedCluster %s with template %s", sdName, sdTemplate))

			templateBy(templates.TemplateAzureStandaloneCP, "creating a ManagedCluster")
			sd := managedcluster.GetUnstructured(templates.TemplateAzureStandaloneCP, sdName, sdTemplate)
			...
			hdName := managedcluster.GenerateClusterName(fmt.Sprintf("azure-hosted-%d", i))
			hdTemplate := providerConfig.Hosted.Template
			templateBy(templates.TemplateAzureHostedCP, fmt.Sprintf("creating a hosted ManagedCluster %s with template %s", hdName, hdTemplate))
