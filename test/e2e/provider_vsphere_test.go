...
var _ = Context("vSphere Templates", Label("provider:onprem", "provider:vsphere"), Ordered, func() {
	var (
		...
		deleteFuncs            = make(map[string]func() error)
	...
	AfterAll(func() {
		...
		// fails to do so.
		for clusterName, deleteFunc := range deleteFuncs {
	...
	for _, providerConfig := range providerConfigs {
		Context(fmt.Sprintf("Testing VSphere ManagedCluster deployment with the following configuration: %s", providerConfig.String()), func() {
			It("should deploy standalone managed cluster", func() {
				By("creating a managed cluster")
				d := managedcluster.GetUnstructured(templates.TemplateVSphereStandaloneCP, providerConfig.Standalone)
				clusterName := d.GetName()

				deleteFunc := kc.CreateManagedCluster(context.Background(), d)
				deleteFuncs[clusterName] = deleteFunc
